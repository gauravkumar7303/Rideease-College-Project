...
export default function CarsPage() {
  ...
  const fetchCars = async () => {
    ...
    try {
      ...
      if (data.success && data.vehicles) {
        ...
        const formattedCars = data.vehicles.map(vehicle => ({
          id: vehicle._id,
          type: 'car',
          brand: vehicle.brand,
          model: vehicle.model,
          ...
          image: vehicle.images?.[0]?.data || 'https://images.unsplash.com/photo-1621007947382-bb3c3994e3fb?auto=format&fit=crop&w=500',
          features: vehicle.features || ['AC', 'Power Steering'],
  ...
  // Extract unique brands from database cars
  const uniqueBrands = [...new Set(cars.map(car => car.brand))];
  const BRANDS = { cars: uniqueBrands.length ? uniqueBrands : [
    'Maruti Suzuki', 'Hyundai', 'Toyota', 'Honda', 'Tata',
    'Mahindra', 'Ford', 'Volkswagen', 'Skoda', 'MG', 'Kia'
  ]};
        ...
        <div className="section-padding max-w-7xl mx-auto relative z-10">
          ...
          >
            ...
            <div className="flex justify-center gap-8 mt-8">
              <div className="flex items-center gap-2">
                <FaStar className="text-yellow-400" />
                <span className="text-emerald-100">4.7/5 ({cars.reduce((sum, c) => sum + c.reviews, 0)}+ reviews)</span>
              </div>
              <div className="flex items-center gap-2">
                <FaMapMarkerAlt className="text-emerald-300" />
                <span className="text-emerald-100">30+ locations</span>
